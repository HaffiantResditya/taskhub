    ...
    <div class="flex min-h-screen">
        ...
        <aside :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full'"
            ...
            <nav class="px-4 flex-1 overflow-y-auto">
                ...
                {{-- Sidebar Projects --}}
                @php
                    $sidebarProjects = auth()->user()->projects()->withCount('tasks')->latest()->take(8)->get();
                @endphp

                @if ($sidebarProjects->isNotEmpty())
                    <p
                        class="text-[11px] font-semibold text-[#B8B8B5] uppercase
                  tracking-[.06em] px-3 pt-5 pb-2">
                    ...
                    @foreach ($sidebarProjects as $sp)
                      ...
                      {{ request()->is('projects/' . $sp->id) || request()->is('projects/' . $sp->id . '/*')
                          ? 'bg-[#FAFAF8] text-[#1C1C1E] font-semibold'
                          : 'text-[#8A8A8E] hover:bg-[#FAFAF8] hover:text-[#1C1C1E]' }}">
            ...
            {{-- Profil & Logout (Dipindah ke bagian paling bawah sidebar, mt-auto) --}}
        ...
        <div x-show="sidebarOpen" x-transition:enter="transition ease-out duration-200"
            ...
            @click="sidebarOpen = false" class="fixed inset-0 bg-black/40 z-40 lg:hidden" style="display:none;">
        </div>

        {{-- LANGKAH 7: Main content --}}
