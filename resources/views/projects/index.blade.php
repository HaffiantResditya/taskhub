    ...
    @else
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
            ...
            @foreach ($projects as $project)
                {{-- UBAH 1: Ganti tag pembuka <a> menjadi <div> --}}
                <div
                    ...
                    {{-- UBAH 2: Tambahkan "Stretched Link" gaib untuk menutupi seluruh kartu --}}
                    ...
                    <div class="flex items-center gap-2 mb-2 mt-1 relative z-10 pointer-events-none">
                        <span class="w-[10px] h-[10px] rounded-full shrink-0"
                            style="background-color:{{ $project->color }}"></span>
                        <h3
                            class="font-bold text-gray-900 truncate text-[15px] group-hover:text-blue-600 transition-colors">
                            {{ $project->name }}
                        </h3>
                    </div>

                    {{-- Deskripsi --}}
                    <p class="text-sm text-gray-400 line-clamp-2 mb-4 min-h-[40px] relative z-10 pointer-events-none">
                        {{ $project->description ?: 'Tidak ada deskripsi' }}
                    </p>

                    {{-- Footer kartu: counter task + tombol edit/hapus --}}
                    {{-- UBAH 3: Tambahkan relative dan z-10 agar tombol berada di atas stretched link --}}
                    <div class="flex items-center justify-between pt-3 border-t border-[#ECECE9] relative z-10">
                        <span class="text-xs text-gray-400">
                            📋 {{ $project->tasks_count }} tugas
                        </span>

                        {{-- UBAH 4: Hapus onclick="event.stopPropagation()" karena sudah tidak perlu --}}
